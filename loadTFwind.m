...
% 1/sensitivity values are put in p.tf structure
%
...
%   [Pxx,F] = pwelch(dtdata,window,noverlap,p.nfft,p.fs);
%   Pxx = 10*log10(Pxx);
%
...
p.tf.filename = filename;

[A,~] = fscanf(fid,'%f %f',[2,inf]);
p.tf.freq = A(1,:);
p.tf.uppc = A(2,:);    % [dB re uPa(rms)^2/counts^2]

% p.tf.uppc =  - p.tf.vpup - (82.35 + 9.0).*ones(size(p.tf.vpup));
