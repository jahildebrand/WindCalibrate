function [ptimel,mpwrl,mpwrtfl,freql,eltsal,dfreql,dnew,wsnew,fs0l] = calLTSAl49()
...
LTSAFolderl = p.ltsa.LTSAFolder ;
OutFolder = p.harp.OutFolder ;
OutName3 = p.harp.OutName3;
NA = p.harp.NA ;     % number of time slices (spectral averages) to read per raw file
tres = p.harp.tres ;
%
[fn_files, fn_pathname, ~] = uigetfile('*.ltsa','Pick Low',LTSAFolderl,'MultiSelect','on');
...
for k = 1:nltsas    % loop over files
    ...
    if k == 1 % read and set some useful parameters that should be the same across all ltsas
        ...
        p.ltsa.freql = freq; freql = freq;
        dfreq = PARAMS.ltsa.dfreq;
        p.ltsa.dfreql =dfreq; dfreql = dfreq;
        fs0 = PARAMS.ltsa.fs;
        p.ltsa.fs0l = fs0; fs0l = fs0;
    end
end
...
for m = 1:nm    % loop over time average bins
    ...
    for f = 1:nfiles    % loop over files with same month (probably only 2 max)
        ...
        if fs0 == 2000 || fs0 == 200000 || fs0 == 10000 || fs0 == 64000 || fs0 == 50000 || fs0 == 5000
            %  IA = (15+5)*nf;
...
%             disp('error = did not read enough data')
        ...
        fclose(fid);
        [Cltsa, ialtsa, ibltsa] =  intersect(I,eltsal);
        if Cltsa > 0
...
ptimel = ptime; mpwrl = mpwr; mpwrtfl = mpwrtf; freql = freq;  dfreql = dfreq; 
save(fullfile(OutFolder,'WindNoiseMat',OutName3));%,...
   % 'ptimel','mpwrl','mpwrtfl','freql','eltsal') % save all workspace
